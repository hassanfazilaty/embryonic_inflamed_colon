...
hindgut_Mes_dev$celltype <- Idents(hindgut_Mes_dev)

hindgut_Mes_dev <- RenameIdents(hindgut_Mes_dev, `Mtrix fibroblast` = "1", `eFibroblast progenitor` = "2", `Myofibroblast progenitor` = "3", 
                                `CTFs precursor` = "4", `Myofibroblast` = "5", `Myofibroblast progenitor 3` = "6")

hindgut_Mes_dev$clusternumber <- Idents(hindgut_Mes_dev)


mes_dev_df <- as.data.frame(hindgut_Mes_dev@assays[["RNA"]]@data)
...
part <- as.numeric(hindgut_Mes_dev@meta.data[["clusternumber"]])
d <- as.matrix(dist(hindgut_Mes_dev@reductions[["pca"]]@cell.embeddings))
umap <- as.data.frame(hindgut_Mes_dev@reductions[["umap"]]@cell.embeddings)
...
identities_embryo_mes <- levels(hindgut_Mes_dev@active.ident)
my_color_palette_hem <- hue_pal()(length(identities_embryo_mes))

sc@fcol <- my_color_palette_hem

#random
ltr3 <- Ltree(sc)
ltr3 <- compentropy(ltr3)
ltr3 <- projcells(ltr3,cthr=5,nmode=FALSE,fr=FALSE,um=TRUE)

ltr3 <-projback(ltr3,pdishuf=100)
ltr3 <-lineagegraph(ltr3)
ltr3 <-comppvalue(ltr3, pthr=0.1)

plotgraph(ltr3,scthr=0,showCells=FALSE,tp=1)
x <- compscore(ltr3,scthr=0)
