...
r
epi_dev_df <- as.matrix(hindgut_epi_dev@assays[["RNA"]]@counts)

sc <- SCseq(epi_dev_df)
sc <- filterdata(sc,mintotal=1)


part1 <- as.character(hindgut_epi_dev@meta.data[["celltype"]])
part2 <- as.numeric(hindgut_epi_dev@meta.data[["seurat_clusters"]])
d <- as.matrix(dist(hindgut_epi_dev@reductions[["pca"]]@cell.embeddings))
umap <- as.data.frame(hindgut_epi_dev@reductions[["umap"]]@cell.embeddings)
...
#ltr <- Ltree(sc)
#ltr <- compentropy(ltr)
#ltr <- projcells(ltr,cthr=5,nmode=FALSE,fr=FALSE,um=TRUE)
...
identities_embryo_epi <- levels(hindgut_epi_dev@active.ident)
my_color_palette_hee <- hue_pal()(length(identities_embryo_epi))
